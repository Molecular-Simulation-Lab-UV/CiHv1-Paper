...
###### Change here ###########################################################
set refSel  "protein and resid 160 198 227 259 and name CA"; #reference in selectivity Filter D160
set Wrapped 1;# is the trajectory wrapped,  0 no 1 yes
set wrapSel "chain H"              
set outName "LOADS.RMSDA.dat"
#set WatSel "name OH2"
#set WatSel  "chain A and resid 78 162 189 204 and name CB";
set WatSel "chain H and resid 255 and name CA"
source /home/jgarate/work/HV1/MODELS_Ci-HV1_paperCG_with_S0/MDs/ANA/bigdcd.tcl
#set firstDCD 8
#set lastDCD 10
...
for {set i 8} {$i <= 9} {incr i} {
	set dcd($i) /home/dbasaez/NAS-data/RESPALDO_BRUTO_CIHV1_VOLTAGE/N264E_NVT150mV/MD1/eq$i.center.gather.dcd
}

set minZ   -40; #Min Z value for pore
set maxZ    40; #Max Z value for pore
set binNumZ 160; #Numbers of bins along axial Dim
set BinSizeZ  [expr 1.000*($maxZ-$minZ)/$binNumZ]
set rad     30; #Radius of pore
...
#     if { $argc != 2 } {
#	 puts "The P1.loads.tcl script requires  variables  to be inputed."
#	 puts "For example, vmd-1.9.3 -dispdev text -e path/to/program -args path/to/inputpsf  path/to/refdcd"
